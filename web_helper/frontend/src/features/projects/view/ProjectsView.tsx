...
import { useProjectsView } from '@/shared/model/useProjectsView';
...
import { AdvancedFilter } from '@/shared/ui/advanced-filter';
import { MetricsComparisonModal } from '@/widgets/MetricsComparisonModal';
import { RunDetailModal } from '@/widgets/RunDetailModal';
import { BestRunCard } from '@/shared/ui/best-run-card';
import { SavedViewsMenu } from '@/shared/ui/saved-views-menu';
import { SummaryDashboard } from '@/shared/ui/summary-dashboard';
import { RunTimeline } from '@/shared/ui/run-timeline';
import { RunStatusBadge } from '@/shared/ui/run-status-badge';
import { ProjectStatsCard } from '@/shared/ui/project-stats-card';
import { TopRunsTable } from '@/shared/ui/top-runs-table';
import { HyperparameterInsights } from '@/shared/ui/hyperparameter-insights';
import { HyperparamCorrelationCard } from '@/shared/ui/hyperparam-correlation-card';
import { QuickRecommendations } from '@/shared/ui/quick-recommendations';
import type { SavedView } from '@/shared/lib/saved-views';
import type { GridReadyEvent } from 'ag-grid-community';
import { Input } from '@/shared/ui/input';
import { ChartsView } from '@/widgets/ChartsView';
import { AdvancedView } from '@/widgets/AdvancedView';
import { MetricSelector } from '@/shared/ui/metric-selector';
import { AGGridProjectsTable } from '@/shared/ui/AGGridProjectsTable';
import { ColumnSuggestionBanner } from '@/shared/ui/column-suggestion-banner';
import { ColumnMappingDialog } from '@/shared/ui/ColumnMappingDialog';
import { ColumnManager } from '@/shared/ui/column-manager';
...
export function ProjectsView() {
  ...
  const gridRef = useRef<{ api: unknown } | null>(null);
    ...
    if (gridRef.current?.api) {
      ...
      if (allColumns) {
        gridRef.current.api.applyColumnState({
          state: allColumns.map((col: { getId: () => string }) => ({ colId: col.getId(), sort: null })),
      ...
      <div className="space-y-4 pb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-900 mt-1">Browse and manage your experiment _projects</p>
          </div>
        </div>
        {projectsLoading ? (
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="pt-2 border-t space-y-1.5">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-900">Last Activity</span>
  ...
  return (
    <>
          ...
          <TabsContent value="table" className="space-y-2">
            <ErrorBoundary level="section">
              ...
              <AGGridProjectsTable
                ...
                onRowSelectionChange={(rows: Array<{ run_name: string; [key: string]: unknown }>) => setSelectedExperiments(rows.map((r) => r.run_name))}
      ...
      <RunDetailModal
        ...
        run={selectedRunDetail || undefined}
