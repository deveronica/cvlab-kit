...
import { useProjects, useRuns } from '@/shared/model/useProjects';
import { useDevices } from '@/shared/model/useDevices';
import { useQueueStats } from '@/shared/model/useQueue';
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-900">Monitoring</span>
            </div>
            <div className="text-2xl font-bold mt-1">
        ...
        >
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-900">Queue</span>
            </div>
            <div className="text-2xl font-bold mt-1">
        ...
        >
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold mt-1 text-slate-900">Online</div>
            <div className="text-xs text-slate-900">
                ...
                {devices.map(device => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-sm truncate pr-2 text-slate-900">{device.host_id || device.name || device.id}</span>
                      <Badge
                        variant={device.status === 'healthy' ? 'default' : 'secondary'}
                        className={
                          device.status === 'healthy' ? 'bg-green-100 text-green-900 flex-shrink-0' :
                          device.status === 'stale' ? 'bg-yellow-100 text-yellow-900 flex-shrink-0' :
                          'bg-red-50 text-red-900 flex-shrink-0'
                        }
                      >
                        {device.status}
                      </Badge>
                    ...
                    {device.status === 'disconnected' ? (
                      <div className="text-xs text-slate-900 text-center py-2">
                        Device disconnected
                      </div>
                    ) : (
                      <div className="space-y-3">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs text-slate-900">CPU</span>
