...
import { transformForScatterMatrix } from '@/shared/lib/chartDataTransformers';
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: CallbackDataParams) => {
          if (!params.data || !(params.data as any).name) return '';

          const data = params.data as [number, number, number, string];
          const runName = (params.data as any).name;
          const [x, y] = data;
  ...
  const onEvents = useMemo(() => {
    return {
      click: (params: CallbackDataParams) => {
        ...
        if ((params.data as any)?.name) {
          setHighlightedRun((params.data as any).name);
        }
      },
      mouseout: () => {
