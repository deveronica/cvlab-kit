...
import { useDevices, useHeartbeatMutation } from '@/shared/model/useDevices';
import type { Device } from '@/shared/model/types';
...
export function DevicesTable() {
  ...
  const columns = useMemo<ColumnDef<Device>[]>(() => [
    ...
    },
    {
      ...
      header: 'Memory',
      cell: ({ row }) => {
        const used = row.original.memory_used;
        const total = row.original.memory_total;
    ...
    },
    {
      ...
      header: 'VRAM',
      cell: ({ row }) => {
        const used = row.original.vram_used;
        const total = row.original.vram_total;
        if (!used || !total) return '-';
