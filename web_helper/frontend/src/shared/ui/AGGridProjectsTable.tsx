...
import { ColumnStatisticsDialog } from '@shared/ui/column-statistics-dialog';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
interface AGGridProjectsTableProps {
  ...
  activeProject?: string | null; // Active project name for statistics
  ...
  onQuickFilterChange?: (text: string) => void; // Filter change handler
  rowMode?: '1-line' | '2-line' | '3-line'; // Row display _mode (1-line, 2-line, or 3-line)
  onToggleMaximize?: () => void; // Toggle row _mode handler
  onResetFilters?: () => void; // Reset filters handler (called from parent)
  onExportCSV?: () => void; // Export CSV handler (called from parent)
  gridRef?: React.MutableRefObject<AgGridReact | null>; // Grid ref to expose API
...
export function AGGridProjectsTable({
  ...
  highlightedRun = null,
  quickFilterText = '',
  onQuickFilterChange: _onQuickFilterChange,
  rowMode = '2-line',
  ...
  const _calculateColumnWidth = (headerName: string, dataValues: unknown[], targetLines: number = 2): { min: number; default: number; max: number } => {
  ...
  const getMetricDisplayValue = useCallback((value: unknown, metricName: string): number | null => {
    ...
    if (Array.isArray(value)) {
      const nums = value.filter((v: unknown) => typeof v === 'number' && !isNaN(v as number));
  ...
  const MetricHeaderComponent = (_props: IHeaderParams) => {
    ...
    const colDef = _props.column.getColDef() as Record<string, unknown> | undefined;
    const metric = (colDef?.headerComponentParams as Record<string, unknown> | undefined)?.metricName as string || metricName;
  ...
  const columnDefs = useMemo<ColDef[]>(() => {
    ...
    const checkboxColDef: ColDef = {
      ...
      cellRenderer: (_params: ICellRendererParams<ExperimentRow>) => {
      ...
      cols.push({
        ...
        cellRendererSelector: (_params) => {
          ...
          if (_params.data?.isStatisticsRow && _params.value && typeof _params.value === 'object') {
            return {
              component: (_props: { value: unknown }) => {
  ...
  useEffect(() => {
    ...
    const columnState: Array<Record<string, unknown>> = [];
  ...
  useEffect(() => {
    if (gridRef.current?.api && data.length > 0) {
      ...
      if (selectedRows.length > 0) {
        api.forEachNode((node: RowNode<ExperimentRow>) => {
  ...
  useEffect(() => {
    if (gridRef.current?.api && highlightedRun) {
      ...
      api.forEachNode((node: RowNode<ExperimentRow>, index: number) => {
  ...
  // Handle row click
  const onCellClicked = useCallback((event: CellClickedEvent<ExperimentRow>) => {
    // Skip if click is on checkbox column
    if ((event.column as any)?._colId === 'checkbox') {
      return;
    }

    // Skip statistics row
    if ((event.data as any)?.isStatisticsRow) {
      return;
    }
  ...
  const onColumnPinned = useCallback((event: ColumnPinnedEvent<ExperimentRow>) => {
  ...
  const onColumnMoved = useCallback((event: ColumnMovedEvent<ExperimentRow>) => {
