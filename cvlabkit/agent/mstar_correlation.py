...
class MSTARCorrelation(Agent):
    ...
    def evaluate(self):
        ...
        print("-" * 80)
        print(
            f"ECR (Energy Compaction):     Mean={physical_results['ecr_mean']:.4f}, Std={physical_results['ecr_std']:.4f}"
        )
        print(
            f"TV (Total Variation):        Mean={physical_results['tv_mean']:.4f}, Std={physical_results['tv_std']:.4f}"
        )
        print(
            f"Kurtosis:                    Mean={physical_results['kurtosis_mean']:.4f}, Std={physical_results['kurtosis_std']:.4f}"
        )
        print(
            f"Shannon Entropy:             Mean={physical_results['entropy_mean']:.4f}, Std={physical_results['entropy_std']:.4f}"
        )
        print(
            f"GLCM Contrast:               Mean={physical_results['glcm_contrast_mean']:.4f}, Std={physical_results['glcm_contrast_std']:.4f}"
        )
        print(
            f"GLCM Entropy:                Mean={physical_results['glcm_entropy_mean']:.4f}, Std={physical_results['glcm_entropy_std']:.4f}"
        )
        print(
            f"ENL:                         Mean={physical_results['enl_mean']:.4f}, Std={physical_results['enl_std']:.4f}"
        )
        ...
        print("-" * 80)
        print(
            f"Prediction Entropy:          Mean={physical_results['pred_entropy_mean']:.4f}, Std={physical_results['pred_entropy_std']:.4f}"
        )
        print(
            f"Cross Entropy Loss:          Mean={physical_results['ce_loss_mean']:.4f}, Std={physical_results['ce_loss_std']:.4f}"
        )
        print(
            f"Pred Entropy ↔ CE Loss:      ρ = {physical_results['corr_pred_entropy_ce_loss']:+.4f}"
        )
        ...
        print("=" * 80)
        print(
            f"ECR ↔ Prediction Entropy:            ρ = {physical_results['corr_ecr_pred_entropy']:+.4f}"
        )
        print(
            f"TV ↔ Prediction Entropy:             ρ = {physical_results['corr_tv_pred_entropy']:+.4f}"
        )
        print(
            f"Kurtosis ↔ Prediction Entropy:       ρ = {physical_results['corr_kurtosis_pred_entropy']:+.4f}"
        )
        print(
            f"Shannon Entropy ↔ Prediction Entropy: ρ = {physical_results['corr_entropy_pred_entropy']:+.4f}"
        )
        print(
            f"GLCM Contrast ↔ Prediction Entropy:  ρ = {physical_results['corr_glcm_contrast_pred_entropy']:+.4f}"
        )
        print(
            f"GLCM Entropy ↔ Prediction Entropy:   ρ = {physical_results['corr_glcm_entropy_pred_entropy']:+.4f}"
        )
        print(
            f"ENL ↔ Prediction Entropy:            ρ = {physical_results['corr_enl_pred_entropy']:+.4f}"
        )
        ...
        print("=" * 80)
        print(
            f"ECR ↔ CE Loss:                       ρ = {physical_results['corr_ecr_ce_loss']:+.4f}"
        )
        print(
            f"TV ↔ CE Loss:                        ρ = {physical_results['corr_tv_ce_loss']:+.4f}"
        )
        print(
            f"Kurtosis ↔ CE Loss:                  ρ = {physical_results['corr_kurtosis_ce_loss']:+.4f}"
        )
        print(
            f"Shannon Entropy ↔ CE Loss:           ρ = {physical_results['corr_entropy_ce_loss']:+.4f}"
        )
        print(
            f"GLCM Contrast ↔ CE Loss:             ρ = {physical_results['corr_glcm_contrast_ce_loss']:+.4f}"
        )
        print(
            f"GLCM Entropy ↔ CE Loss:              ρ = {physical_results['corr_glcm_entropy_ce_loss']:+.4f}"
        )
        print(
            f"ENL ↔ CE Loss:                       ρ = {physical_results['corr_enl_ce_loss']:+.4f}"
        )
        print("=" * 80 + "\n")
        ...
        if (
            ...
            and accuracy_results["accuracy"] >= target_accuracy
        ):
            print("\n" + "=" * 80)
            print(
                f"🎯 TARGET ACCURACY REACHED: {accuracy_results['accuracy']:.4f} >= {target_accuracy:.4f}"
            )
    ...
    def _save_correlation_results(self, physical_results: dict, accuracy_results: dict):
        ...
        results = {
            ...
            "correlations": {
                ...
                "glcm_contrast_pred_entropy": physical_results[
                    "corr_glcm_contrast_pred_entropy"
                ],
                "glcm_entropy_pred_entropy": physical_results[
                    "corr_glcm_entropy_pred_entropy"
                ],
        ...
        output_file = os.path.join(
            output_dir, f"correlation_epoch_{self.current_epoch:03d}.json"
        )
    ...
    def _save_correlation_summary(self, physical_results: dict, accuracy_results: dict):
        ...
        summary_file = os.path.join(
            output_dir, f"summary_epoch_{self.current_epoch:03d}.txt"
        )
        ...
        with open(summary_file, "w") as f:
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"Validation Accuracy:  {accuracy_results['accuracy']:.4f} ({accuracy_results['accuracy'] * 100:.2f}%)\n"
            )
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"{'ECR (Energy Compaction)':<25s} {physical_results['ecr_mean']:>12.4f} {physical_results['ecr_std']:>12.4f}\n"
            )
            f.write(
                f"{'TV (Total Variation)':<25s} {physical_results['tv_mean']:>12.2f} {physical_results['tv_std']:>12.2f}\n"
            )
            f.write(
                f"{'Kurtosis':<25s} {physical_results['kurtosis_mean']:>12.4f} {physical_results['kurtosis_std']:>12.4f}\n"
            )
            f.write(
                f"{'Shannon Entropy':<25s} {physical_results['entropy_mean']:>12.4f} {physical_results['entropy_std']:>12.4f}\n"
            )
            f.write(
                f"{'GLCM Contrast':<25s} {physical_results['glcm_contrast_mean']:>12.2f} {physical_results['glcm_contrast_std']:>12.2f}\n"
            )
            f.write(
                f"{'GLCM Entropy':<25s} {physical_results['glcm_entropy_mean']:>12.4f} {physical_results['glcm_entropy_std']:>12.4f}\n"
            )
            f.write(
                f"{'ENL':<25s} {physical_results['enl_mean']:>12.4f} {physical_results['enl_std']:>12.4f}\n"
            )
            f.write(
                f"{'Prediction Entropy':<25s} {physical_results['pred_entropy_mean']:>12.4f} {physical_results['pred_entropy_std']:>12.4f}\n\n"
            )
            ...
            f.write("=" * 80 + "\n")
            f.write(
                f"{'Physical Indicator':<35s} {'Spearman ρ':>15s} {'Interpretation':>20s}\n"
            )
            f.write("-" * 80 + "\n")
            ...
            correlations = [
                ...
                (
                    "Kurtosis ↔ Prediction Entropy",
                    physical_results["corr_kurtosis_pred_entropy"],
                ),
                (
                    "Shannon Entropy ↔ Pred. Entropy",
                    physical_results["corr_entropy_pred_entropy"],
                ),
                (
                    "GLCM Contrast ↔ Pred. Entropy",
                    physical_results["corr_glcm_contrast_pred_entropy"],
                ),
                (
                    "GLCM Entropy ↔ Pred. Entropy",
                    physical_results["corr_glcm_entropy_pred_entropy"],
                ),
            ...
            f.write(
                "Positive ρ: As physical indicator increases, prediction entropy increases\n"
            )
            f.write("            (More of this property → Model more uncertain)\n\n")
            f.write(
                "Negative ρ: As physical indicator increases, prediction entropy decreases\n"
            )
    ...
    def _save_per_image_table(self, physical_results: dict):
        ...
        csv_file = os.path.join(
            output_dir, f"per_image_epoch_{self.current_epoch:03d}.csv"
        )
