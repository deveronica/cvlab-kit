...
import { formatRelativeTime } from '@/shared/lib/time-format';
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-slate-900">
          ...
          {safeTotal > 0 && <span className="text-slate-900 font-normal"> / {safeTotal.toFixed(1)}GB</span>}
        </span>
      </div>
...
const StatusBadge = ({ status }: { status: string }) => {
  const variants = {
    healthy: { variant: 'default' as const, className: 'bg-green-700 text-white border-green-700', label: 'Healthy' },
    stale: { variant: 'secondary' as const, className: 'bg-yellow-700 text-white border-yellow-700', label: 'Stale' },
    disconnected: { variant: 'destructive' as const, className: 'bg-red-700 text-white border-red-700', label: 'Disconnected' },
  }[status] || { variant: 'secondary' as const, className: 'bg-secondary text-slate-900 border-border', label: status };

  return (
    <Badge variant={variants.variant} className={`${variants.className} font-medium`}>
      <div className={`mr-1.5 h-2 w-2 rounded-full ${
        status === 'healthy' ? 'bg-green-200 animate-pulse' :
        status === 'stale' ? 'bg-yellow-200' :
        'bg-red-200'
      }`} />
      {variants.label}
    </Badge>
...
export function MonitoringView() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Devices</h1>
        ...
        <Card className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950/30">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="h-5 w-5 text-slate-900 mt-0.5 flex-shrink-0" />
              <div className="flex-1 space-y-2">
                ...
                <div className="bg-yellow-200 dark:bg-yellow-900/50 rounded p-2 font-mono text-xs text-slate-900 flex items-start gap-2">
                  <Terminal className="h-3 w-3 mt-0.5 flex-shrink-0 text-slate-900" />
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {device.status === 'disconnected' && (
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-slate-900 hover:text-destructive"
                      ...
                      {device.gpus.map((gpu) => (
                        <div key={gpu.id} className="space-y-4">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-sm text-slate-900">{gpu.name}</span>
                          </div>
                            ...
                            {gpu.power_usage != null && (
                              <div className="bg-secondary/30 rounded-lg p-3 space-y-1">
                                ...
                                <div className="text-2xl font-bold">
                                  ...
                                  <span className="text-sm text-slate-900">W</span>
                                </div>
                              </div>
                            )}
                          </div>
          ...
          <div className="col-span-full">
            <Card>
              <CardContent className="p-12 text-center">
                <p className="text-slate-900">No devices found</p>
              </CardContent>
            </Card>
          </div>
        )}
