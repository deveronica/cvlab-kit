...
export function ComponentsMode({ breadcrumb, onBreadcrumbUpdate }: ComponentsModeProps) {
  ...
  useEffect(() => {
    if (selectedComponent) {
      ...
      // Update breadcrumb when component is selected
      onBreadcrumbUpdate([
        { level: 'root', label: 'cvlabkit', path: '' },
        { level: 'package', label: selectedComponent.category, path: selectedComponent.category },
        { level: 'file', label: `${selectedComponent.name}.py`, path: selectedComponent.path },
      ]);
    }
  }, [selectedComponent, onBreadcrumbUpdate]);

  const fetchVersions = async (category: string, name: string) => {
    try {
      const response = await fetch(`/api/ui/${name}?include_content=true`);
      const result = await response.json();
  ...
  const handleActivateVersion = async (hash: string) => {
    try {
      const response = await fetch('/api/ui/activate', {
        method: 'POST',
                    ...
                    {groupedSymbols.classes.map((cls) => (
                      <div key={cls.name} className="mb-1">
                        <button
                          ...
                          className="w-full flex items-center gap-1.5 px-2 py-1 rounded hover:bg-accent/50 text-sm transition-colors duration-200 group"
                        >
                          ...
                          <div className="ml-3 pl-2 border-l border-border/50">
                            {cls.methods.map((method) => (
                              <button
                                ...
                                className="w-full flex items-center gap-1.5 px-2 py-0.5 rounded hover:bg-accent/50 text-sm transition-colors duration-200 group"
                              >
              ...
              <div className="p-1.5 space-y-1">
                {versions.map((version, index) => (
                  <div
                    key={version.hash}
                    className={cn(
                      "px-2 py-1.5 rounded transition-all duration-200 group",
