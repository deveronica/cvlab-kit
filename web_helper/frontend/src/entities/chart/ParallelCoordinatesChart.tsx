...
const ParallelCoordinatesChart: React.FC<ParallelCoordinatesChartProps> = memo(({
    ...
    return {
      tooltip: {
        ...
        formatter: (params: CallbackDataParams) => {
          const p = params as CallbackDataParams & { data?: unknown[]; dataIndex?: number };
          if (!p.data) return '';

          // Show only key metrics (last 4 dimensions) for concise view
          const values = p.data;
          const runName = runNames[p.dataIndex || 0] || `Run ${(p.dataIndex || 0) + 1}`;
  ...
  const onEvents = useMemo(() => ({
    click: (params: CallbackDataParams) => {
      const p = params as CallbackDataParams & { dataIndex?: number };
      if (onRunSelect && p.dataIndex !== undefined && runNames[p.dataIndex]) {
        onRunSelect(runNames[p.dataIndex]);
      }
    },
