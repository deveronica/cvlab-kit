...
import { applyFilters, type FilterRule } from '@/shared/ui/advanced-filter';
...
export function useProjectsView() {
  ...
  const fetchRunConfiguration = async (run: Record<string, unknown>) => {
    ...
    try {
      const response = await apiClient.getRunConfig(activeProject, run.run_name as string, { signal });
  ...
  const fetchRunLogs = async (run: Record<string, unknown>) => {
    ...
    try {
      const response = await apiClient.getRunLogs(activeProject, run.run_name as string, { signal });
      // Check if request was aborted
      if (signal.aborted) return;
  ...
  const fetchRunDetails = async (run: Record<string, unknown>) => {
    ...
    // Cancel previous metrics request
    ...
    // These functions have their own AbortControllers
    fetchRunConfiguration(run);
    fetchRunLogs(run);

    try {
      const runName = typeof run.run_name === 'string' ? run.run_name : '';
      const metricsResponse = await apiClient.getRunMetrics(activeProject, runName, { signal });
      // Check if request was aborted
      if (signal.aborted) return;
      ...
      const runWithMetrics = {
        ...
        metrics: {
          final: (run as Record<string, unknown>).final_metrics || {},
          max: {},
          min: {},
          mean: {},
          timeseries
