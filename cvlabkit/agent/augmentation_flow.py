"""Agent for training Rectified Flow on weak→strong augmentation pairs.

This agent learns the distribution flow from weakly augmented images (x_0)
to strongly augmented images (x_1) using Rectified Flow. The trained model
...
class AugmentationFlow(Agent):
    """Agent for learning augmentation flow using Rectified Flow.

    Trains a flow model to transform weakly augmented images to strongly
    augmented images. The model learns the ODE path x_0 → x_1 where:
    - x_0: Weak augmentation (e.g., resize, normalize)
    - x_1: Strong augmentation (e.g., RandAugment)
    ...
    def setup(self):
        ...
        # Create transforms
        self.transform_weak = self.create.transform.weak()
        self.transform_strong = self.create.transform.strong()
    ...
    def train_step(self, batch):
        ...
        # Sample random times for interpolation
        times = torch.rand(len(x_0), device=self.device)
    ...
    def sample_and_save(self, fname):
        ...
        with torch.no_grad():
            ...
            # Save using checkpoint component or fallback
            if self.image_checkpoint is not None:
                save_path = self.image_checkpoint.save_comparison(
